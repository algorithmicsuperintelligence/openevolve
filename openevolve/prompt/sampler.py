...
from openevolve.utils.format_utils import format_metrics_safe, format_score
...
class PromptSampler:
    ...
    def build_prompt(
        ...
        **kwargs: Any,
        ...
        user_message = user_template.format(
            metrics=metrics_str,
            fitness_score=format_score(fitness_score),
    ...
    def _identify_improvement_areas(
        ...
        if previous_programs:
            ...
            current_score_str = format_score(current_fitness)
            prev_score_str = format_score(prev_fitness)

            if current_fitness > prev_fitness:
                key = "fitness_improved"
                kwargs = dict(prev=prev_score_str, current=current_score_str)
            elif current_fitness < prev_fitness:
                key = "fitness_declined"
                kwargs = dict(prev=prev_score_str, current=current_score_str)
            # Essentially unchanged
            elif np.allclose(current_fitness, prev_fitness, atol=1e-6, equal_nan=True):
                key = "fitness_stable"
                kwargs = dict(current=current_score_str)
            else:
                key = None

            if key:
                improvement_areas.append(self.template_manager.get_fragment(key, **kwargs))
