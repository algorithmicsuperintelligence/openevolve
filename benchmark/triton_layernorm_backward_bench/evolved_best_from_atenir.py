...
def _layernorm_bwd_dx_kernel(
    ...
    x_vals = tl.load(x_ptr + row * x_s0 + cols * x_s1, mask=mask, other=0.0).to(tl.float32)
    dy_vals = tl.load(dy_ptr + row * dy_s0 + cols * dy_s1, mask=mask, other=0.0).to(tl.float32)
    w_vals = tl.load(weight_ptr + cols * weight_s0, mask=mask, other=0.0).to(tl.float32)

    mean = tl.sum(x_vals, axis=0) / N
    xhat = x_vals - mean
    xhat = tl.where(mask, xhat, 0.0)
    rstd = tl.rsqrt(tl.sum(xhat * xhat, axis=0) / N + eps)
    xhat = xhat * rstd

    one = dy_vals * w_vals
    c1 = tl.sum(one, axis=0) / N
    c2 = tl.sum(one * xhat, axis=0) / N
    dx_vals = (one - c1 - xhat * c2) * rstd

    tl.store(dx_ptr + row * dx_s0 + cols * dx_s1, dx_vals, mask=mask)
...
def _layernorm_bwd_param_kernel(
    ...
    x_ptrs = x_ptr + rows[:, None] * x_s0 + cols[None, :] * x_s1
    dy_ptrs = dy_ptr + rows[:, None] * dy_s0 + cols[None, :] * dy_s1

    x_vals = tl.load(x_ptrs, mask=mask, other=0.0).to(tl.float32)
    dy_vals = tl.load(dy_ptrs, mask=mask, other=0.0).to(tl.float32)

    mean = tl.sum(x_vals, axis=1) / N
    xhat = x_vals - mean[:, None]
    xhat = tl.where(mask, xhat, 0.0)
    rstd = tl.rsqrt(tl.sum(xhat * xhat, axis=1) / N + eps)
    xhat = xhat * rstd[:, None]

    acc_dbias = tl.sum(dy_vals, axis=0)
    acc_dweight = tl.sum(dy_vals * xhat, axis=0)

    tl.atomic_add(dbias_ptr + cols * dbias_s0, acc_dbias, mask=cols < N)
    tl.atomic_add(dweight_ptr + cols * dweight_s0, acc_dweight, mask=cols < N)


@triton.jit
def _zero_two_kernel(
    dweight_ptr,
    dbias_ptr,
    N: tl.constexpr,
    dweight_s0,
    ...
    tl.store(dweight_ptr + cols * dweight_s0, z, mask=mask)
    tl.store(dbias_ptr + cols * dbias_s0, z, mask=mask)
...
def _layernorm_bwd_fused_kernel(
    ...
    one = dy_vals * w_vals[None, :]
    c1 = tl.sum(one, axis=1) / N
    c2 = tl.sum(one * xhat, axis=1) / N
    dx_vals = (one - c1[:, None] - xhat * c2[:, None]) * rstd[:, None]
    ...
    tl.atomic_add(dbias_ptr + cols * dbias_s0, acc_dbias, mask=col_mask)
    tl.atomic_add(dweight_ptr + cols * dweight_s0, acc_dweight, mask=col_mask)
...
def _launch_layernorm_bwd_param(
    ...
    _layernorm_bwd_param_kernel[grid](
        dy,
        x,
        dweight,
        dbias,
...
def _launch_layernorm_bwd_fused(
    dy: torch.Tensor,
    x: torch.Tensor,
    weight: torch.Tensor,
    dx: torch.Tensor,
    ...
    _layernorm_bwd_fused_kernel[(triton.cdiv(m, block_m),)](
        dy,
        x,
        weight,
        dx,
...
def layernorm_backward_triton(dy, x, weight, bias, eps=1e-5):
    ...
    if x.dtype is torch.float16 and dy.dtype is torch.float16 and weight.dtype is torch.float16:
        _launch_layernorm_bwd_fused(dy, x, weight, dx, dweight, dbias, eps)
    else:
