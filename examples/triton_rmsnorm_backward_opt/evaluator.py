"""Evaluator for Triton RMSNorm backward-dx optimization."""
...
CORRECTNESS_CASES = [
    TestCase(128, 1024, "float32", 2e-5, 2e-5),
    TestCase(256, 2048, "float32", 2e-5, 2e-5),
    TestCase(64, 4096, "float32", 2e-5, 2e-5),
    TestCase(512, 1024, "float16", 2e-3, 2e-3),
    TestCase(256, 4096, "float16", 2e-3, 2e-3),
]

BENCHMARK_CASES = [
    TestCase(1024, 1024, "float16", 2e-3, 2e-3),
    TestCase(2048, 2048, "float16", 2e-3, 2e-3),
    TestCase(1024, 4096, "float16", 2e-3, 2e-3),
]
...
def _torch_oracle(torch_module, dy, x, weight):
    x_f32 = x.float()
    dy_f32 = dy.float()
    weight_f32 = weight.float()
    inv_rms = torch_module.rsqrt(torch_module.mean(x_f32 * x_f32, dim=-1, keepdim=True) + EPS)
    grad_normed = dy_f32 * weight_f32
    projection = torch_module.sum(grad_normed * x_f32, dim=-1, keepdim=True)
    n_cols = x.shape[-1]
...
def _validate_api(module) -> Callable:
    fn = getattr(module, "rmsnorm_backward_dx_triton", None)
    if fn is None or not callable(fn):
        raise AttributeError(
            "candidate must define callable rmsnorm_backward_dx_triton(dy, x, weight, eps)"
        )
    return fn
...
def _run_correctness(torch_module, fn: Callable, cases: List[TestCase]):
    ...
    for case in cases:
        try:
            ...
            dy, x, weight = _make_inputs(torch_module, case)
            expected = _torch_oracle(torch_module, dy, x, weight)
            actual = fn(dy, x, weight, EPS)
            ...
            reports.append(
                {
                    ...
                    "rtol": case.rtol,
                    "hint": (
                        "RMSNorm dx must compute dy*weight*inv_rms - "
                        "x*inv_rms^3*sum(dy*weight*x)/hidden_size with fp32 reductions"
                    ),
                }
            )
        except Exception as exc:
...
def _benchmark_ms(torch_module, fn: Callable, case: TestCase, warmup: int = 10, reps: int = 50):
    dy, x, weight = _make_inputs(torch_module, case)
