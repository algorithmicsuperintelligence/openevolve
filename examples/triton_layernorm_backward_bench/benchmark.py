"""Latency benchmark for the LayerNorm backward sample."""
...
def _pytorch_autograd_step(dy: torch.Tensor, x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> None:
...
def run_case(case: TestCase) -> dict[str, float | str | list[int]]:
    ...
    triton_ms = _median_ms(lambda: layernorm_backward_naive_triton(dy, x, weight, bias, EPS))
    pytorch_ms = _median_ms(lambda: _pytorch_autograd_step(dy, x, weight, bias))
    return {
