...
```bash
python -m examples.triton_backward_bench_common.builder_cli emit-spec examples/triton_layernorm_backward_bench
python -m examples.triton_backward_bench_common.builder_cli emit-prompts examples/triton_layernorm_backward_bench
python -m examples.triton_backward_bench_common.builder_cli verify examples/triton_layernorm_backward_bench
```

Run API-based Stage 1 synthesis on a GPU node:

```bash
python -m examples.triton_backward_bench_common.builder_cli synthesize \
  examples/triton_layernorm_backward_bench \
